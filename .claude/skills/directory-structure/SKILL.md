...
- **Bottom pop-up modals (confirm moments).** The customer app's confirm-dialog contract is a centered modal — never a bottom slide-up sheet. Every confirm-modal / dialog / short-form overlay in `apps/customer-app` MUST use `Modal` with `animationType="none"`, `justifyContent: "center"`, `alignItems: "center"`, `maxWidth: 420`, `radii.md` (8px) corners, no grabber bar, no `flex-end` anchor, no `slide` animation. The native `Modal` animation is disabled — the inner surface is wrapped in `Animated.View` from `react-native-reanimated` and animates with a scale-up + fade (scale 0.92 → 1.0, opacity 0 → 1, 180ms in / 120ms out, spring on the scale). The amount-entry modal (`RedemptionAmountSheet.tsx`) and the cancel confirm modal (`MerchantRedemptionConfirmSheet.tsx`) are the canonical centered examples. **Exception — long scroll-view surfaces may stay as bottom sheets.** A modal whose body is a paginated list or a long scroll area (e.g. `ActivitiesModal.tsx`) is allowed to keep `animationType="slide"`, `justifyContent: "flex-end"`, a grabber pill, and rounded top corners — it behaves like a scroll view, not a dialog, and would cramp at `maxWidth: 420`. Why: the centered modal gives the user a calm moment to read destructive-action copy before confirming; the bottom slide-up felt too quick to dismiss and made confirmation moments feel like they skipped the actual confirm step. The scale + fade entry feels iOS-style centered-modal-canonical, not sheet-canonical — the surface "opens" into place rather than sliding up from below. How to apply: before merging any UI change in `apps/customer-app`, run `grep -rn "justifyContent.*flex-end" apps/customer-app/src/app/` and `grep -rn "animationType.*slide" apps/customer-app/src/app/`. For each match, classify it: confirm-modal matches (short copy + ≤2 CTAs) MUST be migrated to the centered modal style. Scroll-view matches (a `Modal` whose body is a `FlatList` or a long scroll surface) MAY remain a bottom sheet.
